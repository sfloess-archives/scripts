...
addDistros() {
    ...
    distro-add        CentOS-5-x86_64          /root/distro/iso/CentOS-5.11-x86_64-bin-DVD-1of2.iso           --autoinstall-meta="${ENTERPISE_KSMETA_DATA}"
    distro-add        CentOS-6-x86_64          /root/distro/iso/CentOS-6.9-x86_64-bin-DVD1.iso                --autoinstall-meta="${ENTERPISE_KSMETA_DATA}"
    distro-add        CentOS-7-x86_64          /root/distro/iso/CentOS-7-x86_64-Everything-1708.iso           --autoinstall-meta="${ENTERPISE_KSMETA_DATA}"

    distro-add        RHEL-5-x86_64            /root/distro/iso/rhel-server-5.11-x86_64-dvd.iso               --autoinstall-meta="${ENTERPISE_KSMETA_DATA}"
    distro-add        RHEL-6-x86_64            /root/distro/iso/rhel-server-6.9-x86_64-dvd.iso                --autoinstall-meta="${ENTERPISE_KSMETA_DATA}"
    distro-add        RHEL-7-x86_64            /root/distro/iso/rhel-server-7.4-x86_64-dvd.iso                --autoinstall-meta="${ENTERPISE_KSMETA_DATA}"

    distro-add        Fedora-27-x86_64         /root/distro/iso/Fedora-Server-dvd-x86_64-27-1.6.iso           --autoinstall-meta="${FEDORA_KSMETA_DATA}"

    distro-add-atomic CentOS-7-Atomic-x86_64   /root/distro/iso/CentOS-Atomic-Host-7-Installer.iso            --autoinstall-meta="${ENTERPISE_KSMETA_DATA}" --arch="x86_64" --os-version="rhel7"
    distro-add-atomic RHEL-7-Atomic-x86_64     /root/distro/iso/rhel-atomic-installer-7.4.4-1.x86_64.iso      --autoinstall-meta="${ENTERPISE_KSMETA_DATA}" --arch="x86_64" --os-version="rhel7"
    distro-add-atomic Fedora-27-Atomic-x86_64  /root/distro/iso/Fedora-Atomic-ostree-x86_64-27-20180212.2.iso --autoinstall-meta="${FEDORA_KSMETA_DATA}"    --arch="x86_64" --os-version="fedora27"

    distro-add-rhev   RHEVH-4-x86_64           /root/distro/iso/RHVH-4.1-20180128.0-RHVH-x86_64-dvd1.iso      --autoinstall-meta="${ENTERPISE_KSMETA_DATA}" --arch="x86_64" --os-version="rhel7" --breed="redhat"
}
...
addProfiles() {
    ...
    cobbler-exec profile add --name="CentOS-5-x86_64"         --distro="CentOS-5-x86_64"         --repos="${CENTOS_5_REPOS}" --autoinstall="${STANDARD_KICKSTART}"
    cobbler-exec profile add --name="CentOS-6-x86_64"         --distro="CentOS-6-x86_64"         --repos="${CENTOS_6_REPOS}" --autoinstall="${STANDARD_KICKSTART}"
    cobbler-exec profile add --name="CentOS-7-x86_64"         --distro="CentOS-7-x86_64"         --repos="${CENTOS_7_REPOS}" --autoinstall="${STANDARD_KICKSTART}"

    cobbler-exec profile add --name="RHEL-5-x86_64"           --distro="RHEL-5-x86_64"           --repos="${RHEL_5_REPOS}" --autoinstall="${STANDARD_KICKSTART}"
    cobbler-exec profile add --name="RHEL-6-x86_64"           --distro="RHEL-6-x86_64"           --repos="${RHEL_6_REPOS}" --autoinstall="${STANDARD_KICKSTART}"
    cobbler-exec profile add --name="RHEL-7-x86_64"           --distro="RHEL-7-x86_64"           --repos="${RHEL_7_REPOS}" --autoinstall="${STANDARD_KICKSTART}"

    cobbler-exec profile add --name="Fedora-27-x86_64"        --distro="Fedora-27-x86_64"        --repos="${FEDORA_REPOS}" --autoinstall="${STANDARD_KICKSTART}"

    cobbler-exec profile add --name="RHEVH-4-x86_64"          --distro="RHEVH-4-x86_64"          --repos="${RHEL_7_REPOS}" --autoinstall="${STANDARD_KICKSTART}"

    cobbler-exec profile add --name="CentOS-7-Atomic-x86_64"  --distro="CentOS-7-Atomic-x86_64"  --autoinstall="${CENTOS_ATOMIC_KICKSTART}"
    cobbler-exec profile add --name="RHEL-7-Atomic-x86_64"    --distro="RHEL-7-Atomic-x86_64"    --autoinstall="${RHEL_ATOMIC_KICKSTART}"
    cobbler-exec profile add --name="Fedora-27-Atomic-x86_64" --distro="Fedora-27-Atomic-x86_64" --autoinstall="${FEDORA_ATOMIC_KICKSTART}"
}
...
addHosts() {
    ...
    cobbler-exec system add --name="host-1" --hostname="host-1" --profile="CentOS-7-x86_64" --interface="eth0" --mac-address="00:14:22:2A:AF:F8" --virt-type="xenpv" --autoinstall-meta='lvmDisks="sda sdb"'
    cobbler-exec system add --name="host-2" --hostname="host-2" --profile="RHEL-7-x86_64"   --interface="eth0" --mac-address="00:19:B9:1F:34:B6" --virt-type="kvm"   --autoinstall-meta='lvmDisks="sda sdb sdc"'
    cobbler-exec system add --name="host-3" --hostname="host-3" --profile="RHEL-7-x86_64"   --interface="eth0" --mac-address="00:21:9B:32:5F:78" --virt-type="kvm"   --autoinstall-meta='lvmDisks="sda sdb sdc"'
}
...
addVms() {
    ...
    cobbler-exec system add --name="centos-7-atomic-kvm"  --hostname="centos-atomic-kvm" --profile="CentOS-7-Atomic-x86_64"  --interface="eth0"  --mac-address="random" --virt-type="kvm" --virt-file-size="20" --virt-ram="2048" --virt-bridge="bridge0" --autoinstall-meta='hostname="centos-7-atomic"'
    cobbler-exec system add --name="fedora-27-atomic-kvm" --hostname="fedora-atomic-kvm" --profile="Fedora-27-Atomic-x86_64" --interface="eth0"  --mac-address="random" --virt-type="kvm" --virt-file-size="20" --virt-ram="2048" --virt-bridge="bridge0" --autoinstall-meta='hostname="fedora-27-atomic"'
    cobbler-exec system add --name="rhel-7-atomic-kvm"    --hostname="rhel-atomic-kvm"   --profile="RHEL-7-Atomic-x86_64"    --interface="eth0"  --mac-address="random" --virt-type="kvm" --virt-file-size="20" --virt-ram="2048" --virt-bridge="bridge0" --autoinstall-meta='hostname="rhel-7-atomic"'

    cobbler-exec system add --name="centos-7-atomic-xen"  --hostname="centos-atomic-xen" --profile="CentOS-7-Atomic-x86_64"  --interface="eth0"  --mac-address="random" --virt-type="xenpv" --virt-file-size="20" --virt-ram="2048" --virt-bridge="bridge0" --autoinstall-meta='hostname="centos-7-atomic"'
    cobbler-exec system add --name="fedora-27-atomic-xen" --hostname="fedora-atomic-xen" --profile="Fedora-27-Atomic-x86_64" --interface="eth0"  --mac-address="random" --virt-type="xenpv" --virt-file-size="20" --virt-ram="2048" --virt-bridge="bridge0" --autoinstall-meta='hostname="fedora-27-atomic"'
    cobbler-exec system add --name="rhel-7-atomic-xen"    --hostname="rhel-atomic-xen"   --profile="RHEL-7-Atomic-x86_64"    --interface="eth0"  --mac-address="random" --virt-type="xenpv" --virt-file-size="20" --virt-ram="2048" --virt-bridge="bridge0" --autoinstall-meta='hostname="rhel-7-atomic"'
